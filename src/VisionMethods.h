...
double distFromGearPeg() { //Find distance from the gear peg, in inches.
	...
	std::vector<std::vector<cv::Point>> contours = filteredContours(CAMERA_PORT);
...
double distFromContour(std::vector<cv::Point> contour) {
	return 48.0*boundingRect(contour).height/targetHeight4FeetFromGearPeg;
}
...
std::string initialOrientation() {
	...
	std::vector<cv::Point> centers = contourCenters(filteredContours(CAMERA_PORT));
